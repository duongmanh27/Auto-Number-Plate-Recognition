...
yolo_LP_detect = torch.hub.load("yolov5", 'custom', path='models/LP_detector_nano_61.pt',
    force_reload=True,
    source='local')
yolo_license_plate = torch.hub.load("yolov5", 'custom', path='models/LP_ocr_nano_62.pt',
    force_reload=True,
    source='local')
        ...
        if car_id != -1 :
            ...
            for cc in range(2) :
                for ct in range(2) :
                    ...
                    # Cập nhật kết quả trong batch: so sánh trong 10 frame hiện tại
                    # if car_id not in batch_results :
                    ...
                    # else :
                    #     current_batch_score = batch_results[car_id]['license_plate']['text_score']
                    #     if current_batch_score is None and text_score is not None :
                    #         batch_results[car_id] = {
                    #             'vehicle' : {'bbox' : [x_car1, y_car1, x_car2, y_car2]},
                    #             'license_plate' : {'bbox' : [x1, y1, x2, y2],
                    #                                'text' : text,
                    ...
                    #             'vehicle_crop' : vehicle_crop
                    #         }
                    #     elif text_score is not None and current_batch_score is not None and text_score > current_batch_score :
                    #         batch_results[car_id] = {
                    #             'vehicle' : {'bbox' : [x_car1, y_car1, x_car2, y_car2]},
                    #             'license_plate' : {'bbox' : [x1, y1, x2, y2],
                    #                                'text' : text,
        ...
        for lost in lost_ids :
            if lost in best_results and lost not in saved_car_ids :
                ...
                if vehicle_crop is not None :
                    ...
                    print(f"Lưu ảnh xe của car_id {lost} vào file {filename}")
    if frame_nmr % 10 == 9 :
