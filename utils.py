...
def compute_skew(image, center_thres) :
    # Kiểm tra ảnh không rỗng trước khi tiến hành
    ...
    image = cv.medianBlur(image, 3)  # Median Blur có thể gây lỗi nếu ảnh không hợp lệ
...
def deskew(image, change_cons, center_thres) :
    ...
    if image is None or image.size == 0 :
        ...
        return image  # Trả về ảnh gốc mà không xử lý
